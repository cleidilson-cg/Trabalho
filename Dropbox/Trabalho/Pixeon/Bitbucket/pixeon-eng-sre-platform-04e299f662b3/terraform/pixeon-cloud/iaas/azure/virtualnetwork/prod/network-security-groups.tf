...
resource "azurerm_resource_group" "network_security_group" {
  name     = "pixeon-firewall"
  location = "${var.stack_location}"
}

resource "azurerm_network_security_group" "pixeon_cloud_firewall" {
  name                = "pixeon-cloud-firewall"
  location            = "${azurerm_resource_group.network_security_group.location}"
  resource_group_name = "${azurerm_resource_group.network_security_group.name}"
}

resource "azurerm_network_security_rule" "pixeon_cloud_inbound" {
  name                        = "pixeon-cloud-firewall-inbound"
  priority                    = 100
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "*"
  ...
  resource_group_name         = "${azurerm_resource_group.network_security_group.name}"
  network_security_group_name = "${azurerm_network_security_group.pixeon_cloud_firewall.name}"
}

resource "azurerm_network_security_rule" "pixeon_cloud_outbound" {
  name                        = "pixeon-cloud-firewall-outbound"
  priority                    = 100
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "*"
